...
satisfaction_median = float(pd.to_numeric(df_no_missing["satisfaction_rating"], errors="coerce").median())
df_no_missing["satisfaction_rating"] = pd.to_numeric(
    df_no_missing["satisfaction_rating"], errors="coerce"
...
# Business rationale: use forward-fill to preserve row volume while inferring from nearby records.
...
# Fill missing loyalty with "Bronze" as conservative default tier.
df_no_missing["loyalty_status"] = df_no_missing["loyalty_status"].fillna("Bronze")
# Fill missing last name with "Unknown" so downstream text cleaning can run safely.
df_no_missing["last_name"] = df_no_missing["last_name"].fillna("Unknown")
# Fill missing phone with placeholder preserving standardized format possibility.
df_no_missing["phone"] = df_no_missing["phone"].fillna("0000000000")
# Fill missing age using median age to keep records available for segmentation.
...
df_text_cleaned["preferred_category"] = (
    df_text_cleaned["preferred_category"].astype(str).str.strip().str.title()
)
...
def normalize_phone(phone_val: str) -> str:
    digits = "".join(ch for ch in str(phone_val) if ch.isdigit())
    if len(digits) == 10:
        return f"({digits[:3]}) {digits[3:6]}-{digits[6:]}"
    return np.nan


df_text_cleaned["phone"] = df_text_cleaned["phone"].apply(normalize_phone)


# TODO 5: Remove Duplicates
# 5.1 Identify duplicate records
# REQUIRED: Store duplicate count in variable 'duplicate_count' (int)
# Using customer_id as business key.
...
print("- Data Type Issues: mixed date formats, currency symbols/commas in total_spent, inconsistent numeric parsing")
...
for loyalty, avg_value in avg_spent_by_loyalty.items():
    print(f"  - {loyalty}: ${avg_value:,.2f}")
print(f"- Top Category: {top_category} with ${top_category_revenue:,.2f} revenue")
print(f"- Satisfaction vs Spend Correlation: {satisfaction_spend_corr:.3f}")
